...
struct NetworkManager {
    ...
    enum URLString {
        ...
        var stringValue: String {
            ...
            case .movieList(let orderType): //http://connect-boxoffice.run.goorm.io/movies?order_type=1
                return URLString.base + "movies?order_type=" + String(orderType)
            case .commentList(let movieId): // http://connect-boxoffice.run.goorm.io/comments?movie_id=5a54c286e8a71d136fb5378e
    ...
    static func fetchMovieList(orderType: MovieOrderType, completion: @escaping (MovieList?, Error?) -> Void) {
        taskForGETRequest(url: URLString.movieList(orderType.rawValue).url, responseType: MovieList.self) { response, error in
            guard let response = response else {
                completion(nil, error)
                return
            }
    ...
    static func fetchCommentList(movieId: String, completion: @escaping (CommentList?, Error?) -> Void) {
        taskForGETRequest(url: URLString.commentList(movieId).url, responseType: CommentList.self) { response, error in
            guard let response = response else {
                completion(nil, error)
                return
            }
